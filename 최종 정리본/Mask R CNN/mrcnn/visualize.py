...
#       hsv = [(i / N, 1, brightness) for i in range(N)]
    hsv = [(1 / 1000, 1, brightness) for i in range(N)]
    # hsv = [(346, 96, 93) for i in range(N)]
    colors = list(map(lambda c: colorsys.hsv_to_rgb(*c), hsv))
#     random.shuffle(colors)
    return colors

def random_colors_mask(N, bright=True):
    """
    Generate random colors.
    To get visually distinct colors, generate them in HSV space then
    convert to RGB.
...
def apply_whitemask(image, mask, color, alpha=0.5):
    """Apply the given mask to the image.
    """
    image[...,0] = 255
    image[...,1] = 255
...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    # Generate random colors
    colors = colors or random_colors(81)
...
def display_instances_leesac(image, boxes, masks, class_ids, class_names,
                      scores=None, title="",
                      figsize=(16, 16), ax=None,
                      show_mask=True, show_bbox=True,
                      colors=None, captions=None):
...
# made by Jaemin    
def display_instances_predict_mask( file_name, image, boxes, masks, class_ids, class_names,
                      scores=None, title="", 
                      figsize=(16, 16), ax=None,
                      show_mask=True, show_bbox=True,
                      colors=None, captions=None):
...
#     temp_DIR = 'C:/Users/user/Desktop/temp_img/original500/'
    ...
    for i in range(N):
        ...
        # Mask
        mask = masks[:, :, i]
        ax.imshow(apply_mask(masked_image, mask, color).astype(np.uint8))

        temp = file_name+ f'_{class_names[class_ids[i]]}_{i}.jpg'
        plt.savefig(temp_DIR+temp,bbox_inches='tight')
        plt.close()
        _, ax = plt.subplots(1, figsize=figsize)
        ax.set_ylim(height + 10, -10)
        ax.set_xlim(-10, width + 10)
...
#     ax.imshow(masked_image.astype(np.uint8))
    if auto_show:
        ...
        A method of storing the final image generated using
        the matplotlib using the savefig and close function is implemented.
        The two-line cord below.
        """
...
# made by Sac
def display_instances_predict_whitemask( file_name, image, boxes, masks, class_ids, class_names,
                      scores=None, title="", 
                      figsize=(16, 16), ax=None,
                      show_mask=True, show_bbox=True,
                      colors=None, captions=None):
...
#     temp_DIR = 'C:/Users/user/Desktop/temp_img/white500/'
    ...
    # Generate random colors
    colors = colors or random_colors_mask(81)
    ...
    for i in range(N):
        ...
        # Mask
        mask = masks[:, :, i]
        ax.imshow(apply_whitemask(masked_image, mask, color).astype(np.uint8))
